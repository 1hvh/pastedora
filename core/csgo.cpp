...
bool CSGO::init( ) {
	...
	m_cl = **reinterpret_cast<CClientState***>((*reinterpret_cast<std::uintptr_t**>(m_engine))[12u] + 0x10u);
	...
	m_input = *pattern::find(m_client_dll, XOR("B9 ? ? ? ? 8B 40 38 FF D0 84 C0 0F 85")).add(0x1).as< CInput** >();
	...
	CL_Move = pattern::find(m_engine_dll, XOR("55 8B EC 8B 4D 04 81 EC")).as<uintptr_t>(); 
